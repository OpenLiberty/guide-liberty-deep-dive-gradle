...
Before you begin, Podman needs to be installed before you start the *Persisting Data* module. For installation instructions, refer to the https://podman.io/getting-started/installation[official Podman documentation^]. You will build and run the microservices in containers.
...
* [hotspot=httpPort file=1]`liberty.server.var.'default.http.port'` to `9081`
* [hotspot=httpsPort file=1]`liberty.server.var.'default.https.port'` to `9445`
* [hotspot=contextRoot file=1]`liberty.server.var.'default.context.root'` to `/trial`
...
* update [hotspot=httpPort file=2]`liberty.server.var.'default.http.port'` to `9080`
* update [hotspot=httpsPort file=2]`liberty.server.var.'default.https.port'` to `9443`
* update [hotspot=contextRoot file=2]`liberty.server.var.'default.context.root'` to `/inventory`
...
Additional information about the annotations that MicroProfile metrics provides, relevant metadata fields, and more are available at the https://openliberty.io/docs/latest/reference/javadoc/microprofile-6.0-javadoc.html?package=org/eclipse/microprofile/metrics/annotation/package-frame.html&class=overview-summary.html[MicroProfile Metrics Annotation Javadoc^].
...
Instead of the hard-coded `serverName`, `portNumber`, `user`, and `password` values in the [hotspot=postgresProperties file=1]`properties.postgresql` properties, use `${postgres/hostname}`, `${postgres/portnum}`, `${postgres/username}`, and `${postgres/password}`, which are defined by the [hotspot=variables file=1]`variable` elements.
...
| -e     | Sets environment variables within the container. For example: `-e <VARIABLE_NAME>=<value>`
|===
