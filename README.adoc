...
Podman must be installed before you start the *Persisting Data* module. For installation instructions, refer to the https://podman.io/getting-started/installation[official Podman documentation^]. You will build and run the microservices in containers.
...
In this workshop, the Open Liberty Starter is used to create the starting point of the application. Gradle is used as the selected build tool and the application uses of Jakarta EE 10 and MicroProfile 6.1.
...
Next, extract the `inventory.zip` file on your system. Move the contents of this extracted `inventory` directory to the `start` directory of this project, which is at the following path: `guide-liberty-deepdive-gradle/start/inventory`.
...
[role='command']
```
curl -o inventory.zip 'https://start.openliberty.io/api/start?a=inventory&b=gradle&e=10.0&g=io.openliberty.deepdive&j=17&m=6.1'
...
>   - In the ***Open*** window, select the ***home*** directory from the top dropdown list, and then select the ***/home/project/guide-liberty-deep-dive-gradle/start/inventory*** directory and click the **Open** button.
>   - Wait for the IDE to refresh.
endif::[]
...
Build and deploy the `inventory` microservice to Liberty by running the Gradle `libertyRun` goal:
...
The `libertyRun` argument specifies the Liberty `run` goal, which starts a Liberty instance in the foreground. As part of this phase, a Liberty server runtime is downloaded and installed into the `build/wlp` directory. Additionally, a Liberty instance is created and configured in the `build/wlp/usr/servers/defaultServer` directory, and the application is installed into that Liberty instance by using https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/rwlp_loose_applications.html[loose config^].
...
Although you can start and stop the Liberty instance in the foreground by using the Gradle `libertyRun` task, you can also start and stop the server in the background with the Gradle `libertyStart` and `libertyStop` goals:
...
The Liberty Gradle plug-in includes a `dev` goal that listens for any changes in the project, including application source code or configuration. The Liberty instance automatically reloads the configuration without restarting. This goal allows for quicker turnarounds and an improved developer experience.

If the Liberty instance is running, stop it and restart it in dev mode by running the `libertyDev` goal in the `start/inventory` directory:
...
Dev mode automatically picks up changes that you make to your application and allows you to run tests by pressing the `enter/return` key in the active command-line session. When you’re working on your application, rather than rerunning Gradle commands, press the `enter/return` key to verify your change.
...
The MicroProfile OpenAPI API is included in the [hotspot=mp5 file=0]`microProfile` dependency that is specified in your `build.gradle` file. The [hotspot=mp5 file=1]`microProfile` feature that includes the `mpOpenAPI` feature is also enabled in the `server.xml` configuration file.
...
build.gradle
[source, xml, gradle, role='code_column hide_tags=copyright']
----
include::staging/module-config/build.gradle[]
----

Set the [hotspot=war file=1]`archiveVersion` property to an empty string for the `war` task and add properties for the `HTTP` port, `HTTPS` port, and the `context root` to the `build.gradle` file.
...
The [hotspot=postgresql file=2]`postgresql` dependency under the [hotspot=configurations file=2]`jdbcLib` configuration ensures that Gradle downloads the PostgreSQL library to local project. The [hotspot=copyJDBC file=2]`copyJdbcLibs` task copies the library to the Liberty shared resources directory and ensures that the driver is available for the Liberty server's use each time the project is built. To make the [hotspot file=0]`persistence.xml` file available in the Liberty loose application configuration, set the value for the Gradle [hotspot=setResourcesDir file=2]`resourcesDir` output directory location to be the Gradle [hotspot=setResourcesDir file=2]`destinationDirectory` directory.  The Gradle project generates the resources artifacts into the `resourcesDir` directory and the Java class files into the `destinationDirectory` directory.
...
You can use the Containerfile to try out your application with the PostGreSQL database by running the `devc` goal.

The Open Liberty Maven plug-in includes a `devc` goal that simplifies developing your application in a container by starting dev mode with container support. This goal builds a Podman image, mounts the required directories, binds the required ports, and then runs the application inside of a container. Dev mode also listens for any changes in the application source code or configuration and rebuilds the image and restarts the container as necessary.
...
POSTGRES_IP=`podaman inspect -f "{{.NetworkSettings.IPAddress }}" postgres-container`
mvn liberty:devc \
...
First, create the `test` directory at the `src` directory of your Gradle project.
...
ifdef::cloud-hosted[]
```bash
mkdir -p /home/project/guide-liberty-deep-dive-gradle/start/inventory/src/test/java/it/io/openliberty/deepdive/rest
mkdir /home/project/guide-liberty-deep-dive-gradle/start/inventory/src/test/resources
```
endif::[]
...
Update the Gradle configuration file with the required dependencies.
...
[source, gradle, linenums, role='code_column hide_tags=copyright']
----
include::finish/module-testcontainers/build.gradle[]
----

Add each required [hotspot=testDependenies file=5]`dependency` with `test` scope, including JUnit5, Testcontainers, Log4J, JBoss RESTEasy client, Glassfish JSON, and Vert.x libraries. Also, add the [hotspot=failsafe file=5]`io.openliberty.tools.gradle.Liberty` plugin, so that the integration test can be run by the Gradle `verify` goal.

=== Running the tests

You can run the Gradle `verify` goal, which compiles the java files, starts the containers, runs the tests, and then stops the containers.
...
```bash
sed -i 's=namespace: default=namespace: '"$SN_ICR_NAMESPACE"'=g' /home/project/guide-liberty-deep-dive-gradle/finish/postgres/postgres.yaml
kubectl apply -f /home/project/guide-liberty-deep-dive-gradle/finish/postgres/postgres.yaml
sed -i 's=liberty-deepdive-inventory:1.0-SNAPSHOT=us.icr.io/'"$SN_ICR_NAMESPACE"'/liberty-deepdive--inventory:1.0-SNAPSHOT\n  pullPolicy: Always\n  pullSecret: icr=g' /home/project/guide-liberty-deep-dive-gradle/start/inventory/inventory.yaml
kubectl apply -f /home/project/guide-liberty-deep-dive-gradle/start/inventory/inventory.yaml
```
endif::[]
...
ifdef::cloud-hosted[]
```bash
kubectl delete -f /home/project/guide-liberty-deep-dive-gradle/start/inventory/inventory.yaml
```
endif::[]
...
```bash
kubectl delete -f /home/project/guide-liberty-deep-dive-gradle/start/inventory/inventory.yaml
kubectl delete -f /home/project/guide-liberty-deep-dive-gradle/finish/postgres/postgres.yaml
kubectl delete configmap inv-app-root
kubectl delete secret post-app-credentials
```
endif::[]
